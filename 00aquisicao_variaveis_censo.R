...
packages<-c('ggplot2','readxl','dplyr','sf')
...
for(i in 1:length(files)){
  ...
  df<-df%>%select(CD_GEOCODI,TIPO,CD_GEOCODM,NM_MUNICIP)
  names(df)<-c('cod_setor','tipo','cod_mun','nome_mun','geometry')
...
for(i in 1:length(files)){
  ...
  df[is.na(df)] <- 0
  df<-df%>%filter(Cod_setor %in% coast_setor)%>%mutate(cod_setor=Cod_setor,ndom=V001, mordom=V003, poptotal=V005,cod_mun=Cod_municipio,situacao=Situacao_setor,nome=Nome_do_municipio)
...
for(i in 1:length(files)){
  df<-read_xls(files[i])
  df[is.na(df)] <- 0
  df<-df%>%filter(Cod_setor %in% coast_setor)%>%mutate(cod_setor=Cod_setor,domprop=as.numeric(V006)+as.numeric(V007), 
  ...
  df<-df%>%select(cod_setor,domprop,domalug,cagua,cbanhesg,sbanh,ccoletalixo,cenergia,senergia,homens,mulheres)
  muni[[i]] <- df
}
